...
function Fournisseurs() {
  ...
  const [fournisseurs, setProspects] = useState([]);
  const [fournisseurToEdit, setProspectToEdit] = useState({});
  ...
  const currentProspects = fournisseurs.slice(
    (page - 1) * itemsPerPage,
    page * itemsPerPage
  );

  const handleNext = () => {
    if (page < Math.ceil(fournisseurs.length / itemsPerPage)) {
      setPage(page + 1);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    console.log(fournisseurToEdit);
    try {
      if (!fournisseurToEdit.id) {
        ...
        const response = await axios.put(`${BASE_URL}fournisseurs/${fournisseurToEdit.id}`, formData, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const handleDelete = async (fournisseurId) => {
    setShowActionsIdProsp((prevId) => (prevId === fournisseurId ? null : fournisseurId));
      ...
      <div className="w-full border rounded-lg shadow-md p-0 relative overflow-x-auto">
        ...
        <div className="divide-y divide-gray-200 p-0 h-[600px] overflow-y-auto min-w-[1000px]">
          {isLoading ? (
            ...
            <>
              {currentProspects.map((fournisseur) => (
                <React.Fragment key={fournisseur.id}>
                  <div className="grid grid-cols-[2fr,5fr,5fr,4fr,4fr,3fr,1fr] px-4 py-2 font-normal hover:shadow-md hover:bg-slate-50">
                    ...
                    <Link to={`${fournisseur.id}`}><div>{fournisseur.nom_societe}</div></Link>
                    <Link to={`${fournisseur.id}`}><div>{fournisseur.nom}</div></Link>
                    <Link to={`${fournisseur.id}`}><div>{fournisseur.telephone}</div></Link>
                    <Link to={`${fournisseur.id}`}><div>{fournisseur.email}</div></Link>
                    <Link to={`${fournisseur.id}`}>
                      <div className="text-center" title="Nombre de pièces jointe">
                        {(() => {
                          let nb_link = 0;
                          let nb_linkShow = '';
                          if (fournisseur.assurance) { nb_link = nb_link + 1; }
                          if (fournisseur.cabisse) { nb_link = nb_link + 1; }
                          if (fournisseur.piece_identite) { nb_link = nb_link + 1; }
                          if (fournisseur.contrats) { nb_link = nb_link + 1; }
                          nb_link = nb_link + fournisseur.files.length;
                    ...
                    <div className="relative text-right">
                      <button
                        onClick={() => toggleActions(fournisseur.id)}
              ...
              {fournisseurs.length === 0 && <p className="text-slate-400"><i>Aucun fournisseur disponible.</i></p>}
            </>
          )}
        </div>
      </div>
